...
func main() {
	...
	checkFile := File{}
	bucketerr := bucket.Get(hashString, &checkFile) // no target file was found
	if bucketerr != nil {

	} else {
		// whenever a file got revisoned, file from backup folder are copied back from old version.
		// backup file name convention, <filename>-<revision number>
